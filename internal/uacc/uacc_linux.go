...
func Open(utmpPath, wtmpPath string, username, hostname string, remote [4]int32, ttyName string, tty *os.File) (err error) {
	if ttyName == "" {
...
func Close(utmpPath, wtmpPath string, ttyName string, tty *os.File) (err error) {
	if ttyName == "" {
		ttyName, err = os.Readlink(tty.Name())
		if err != nil {
			return trace.Errorf("failed to resolve soft proc tty link: %v", err)
...
func PutLastlog(app, username, host string, ttyName string, tty *os.File) (err error) {
	if ttyName == "" {
		ttyName, err = os.Readlink(tty.Name())
		if err != nil {
			return trace.Errorf("failed to resolve soft proc tty link: %v", err)
	...
	t := time.Now().Unix()
	status := C.putlastlogentry(C.int64_t(t), C.int(uid), C.CString(app), C.CString(host))
